...
export default function ChatsPage() {
  ...
  useEffect(() => {
    ...
    sock.on('chat:joined', ({ chatId, messages: msgs, memberCount: mc }) => {
      setMessages(msgs || [])
      setMemberCount(mc || 0)
...
function MessageBubble({ msg, isMe }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: isMe ? 'row-reverse' : 'row', gap: 8, alignItems: 'flex-end' }}>
      {!isMe && (
        <div style={{ width: 28, height: 28, borderRadius: '50%', background: `${color}20`, border: `1px solid ${color}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, color, flexShrink: 0, fontFamily: 'var(--font-display)' }}>
          {(msg.userName || '?').charAt(0).toUpperCase()}
        </div>
      )}
      <div style={{ maxWidth: '75%', display: 'flex', flexDirection: 'column', gap: 2, alignItems: isMe ? 'flex-end' : 'flex-start' }}>
        {!isMe && (
          <span style={{ fontSize: 10, color, fontWeight: 700, fontFamily: 'var(--font-display)', paddingLeft: 4 }}>
            {msg.userName}
          </span>
        )}
        <div style={{
          ...
          border: isMe ? '1px solid rgba(124,106,255,0.4)' : '1px solid rgba(255,255,255,0.08)',
          fontSize: 14, color: isMe ? 'white' : 'var(--t1)',
          lineHeight: 1.45, wordBreak: 'break-word',
          boxShadow: isMe ? '0 4px 16px rgba(124,106,255,0.25)' : 'none',
