...
io.on('connection', (socket) => {
  ...
  socket.on('chat:join', async ({ chatId, password }) => {
    try {
      ...
      chatUsers.set(socket.id, { userId: socket.userId, userName: socket.userName, chatId });
      ...
      socket.emit('chat:joined', {
        chatId,
        messages: msgs.map(m => ({ id: m.id, chatId: m.chatId, userId: m.userId, userName: m.userName, text: m.text, ts: m.createdAt })),
        memberCount,
      });
  ...
  socket.on('chat:message', async ({ chatId, text }) => {
    ...
    try {
      ...
      const msg = await ChatMessage.create({
        chatId, userId: socket.userId, userName: socket.userName, text: text.trim(),
      });
      ...
      const out = { id: msg.id, chatId, userId: msg.userId, userName: msg.userName, text: msg.text, ts: msg.createdAt };
