...
async function notifyBanned(user, bannedUntil, reason) {
  const exp = bannedUntil
    ? `до ${new Date(bannedUntil).toLocaleString('ru', { day:'2-digit', month:'2-digit', year:'numeric', hour:'2-digit', minute:'2-digit' })}`
    : 'навсегда';
...
async function notifyPurchase(buyer, seller, productTitle, amount) {
  const sellerName = seller?.firstName || seller?.username || 'Продавец';
  const buyerName  = buyer?.firstName  || buyer?.username  || 'Покупатель';
  await Promise.all([
