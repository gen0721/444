...
router.post('/', auth, async (req, res) => {
  try {
    const { title, description, price, category, subcategory, game, server, images, tags, deliveryData, platform, region, deliveryType, stock } = req.body;
    if (!title || !price || !category) return res.status(400).json({ error: 'Missing required fields' });
    const p = await Product.create({
      sellerId: req.userId, title, description: description || '',
      ...
      platform: platform||null, region: region||null,
      deliveryType: deliveryType||'digital', stock: parseInt(stock)||1
    });
    res.status(201).json(p);
  } catch (e) { console.error('Product create error:', e.message); res.status(500).json({ error: e.message || 'Failed to create' }); }
});
