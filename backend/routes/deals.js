...
async function completeDeal(deal) {
  ...
  // Close deal chat after completion
  try {
    if (deal.adminNote) {
      const { Chat } = require('../models/index');
      const dealChat = await Chat.findByPk(deal.adminNote);
      if (dealChat) {
        await dealChat.update({ isClosed: true, closedReason: 'Сделка завершена' });
        if (global.io) global.io.to(`chat:${deal.adminNote}`).emit('chat:closed', { chatId: deal.adminNote, reason: 'Сделка завершена' });
      }
    }
  } catch (e) { console.warn('Close deal chat error:', e.message); }
}
